...
pub struct ColorChangeableLabel<T> {
...
pub trait CanChangeColor {
    fn set_text_color(&mut self, color: impl Into<KeyOrValue<Color>>);
}

impl<T: Data> CanChangeColor for ColorChangeableLabel<T> {
    fn set_text_color(&mut self, color: impl Into<KeyOrValue<Color>>) {
...
impl<T: TextStorage> ColorChangeableLabel<T> {
    /// Create a new [`RawLabel`].
    ///
...
impl<T: Data> ColorChangeableLabel<T> {
...
impl<T: Data> Widget<T> for ColorChangeableLabel<T> {
...
impl<T> Deref for ColorChangeableLabel<T> {
...
impl<T> DerefMut for ColorChangeableLabel<T> {
